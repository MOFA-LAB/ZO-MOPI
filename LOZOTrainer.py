...
class LOZOTrainer(Trainer):
    ...
    def lowrank_zo_step(self, model, inputs):
        ...
        self.projected_grad = ((loss1 - loss2) / (2 * self.args.zo_eps*args.rank_r)).item()
    ...
    def lowrank_zo_update(self):
        ...
        # Initialize the gradient accumulator dictionary if it doesn't exist
        if not hasattr(self, 'M_buffer'):
            self.M_buffer = {}
        ...
        for name, param in self.named_parameters_to_optim:
            if param.data.ndim >= 2:
                ...
                # Calculate the current gradient estimate
                G = self.projected_grad * (u @ v.t())


                if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
                    param.data = param.data - self._get_learning_rate() * (G + args.weight_decay * param.data)
                else:
                    param.data = param.data - self._get_learning_rate() * G
            else:
                # Resample z for bias
                z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
                if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
