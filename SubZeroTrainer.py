...
def zeropower_via_newtonschulz5_float(G):
    assert G.ndim >= 2 
    a, b, c = (3.4445, -4.7750,  2.0315)
    
    # 1. Cast to Float32 for the iterative inversion
    # This is crucial because BF16 lacks the mantissa precision for 
    # repeated matrix multiplications in the NS loop.
    X = G.float() 
    ...
    # Ensure spectral norm is at most 1
    # The norm calculation is also much more stable in FP32
    X = X / (X.norm(dim=(-2, -1), keepdim=True) + 1e-7)
    ...
    # 2. Cast back to the original type (likely BF16)
    return X.to(G.dtype)
...
class SubZeroTrainer(Trainer):
            ...
            if param.requires_grad:
                ...
                if len(torch.squeeze(param.data).shape) == 2:
                    if self.state.global_step == 0:
                        ...
                        self.p_state[name] = {'U': torch.zeros(param.data.size(0), args.rank_r), 
                                                'V': torch.zeros(args.rank_r, param.data.size(1))}
                  
                    p_state = self.p_state[name]          
                        
                    if self.state.global_step % args.step_interval == 0:
                            # print(args.mode)
                        
                        U, V = fast_svd_method_v2(w_shape=param.data.shape, device=param.device, dtype=param.data.dtype, rank=args.rank_r)
    ...
    def zo_subspace_update(self, model):
        ...
        # 1. Retrieve the current learning rate. 
        # Assuming the optimizer has one param group or we use the first one.
        # If using a scheduler, ensure this reflects the scheduled LR.
        ...
        for name, param, U, V in self.named_parameters_to_optim:
            # Resample z
            if len(torch.squeeze(param.data).shape) == 2:    
                z0 = torch.normal(mean=0, std=1, size=(args.rank_r, args.rank_r), 
            ...
            param.data.add_(-1*current_lr * grad_est)
            # Note: If you intended standard Gradient Descent, change '+' to '-' above.
            
            # Clear grad (good practice, though not strictly necessary if not using optimizer.step)
            param.grad = None 
            
        self.update_steps += 1
        
        # Update the scheduler for the next iteration
