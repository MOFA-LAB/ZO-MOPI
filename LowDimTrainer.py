    ...
    def lowdim_zo_perturb_parameters(self, random_seed=None, scaling_factor=1, sample_idx=0):
        ...
        for name, param in self.named_parameters_to_optim:
            ...
            if param.data.ndim >= 2:
                ...
                resample = (self.step % args.step_interval == 0) or \
                           (name not in self.P_matrices)

                # Resample new P
                if resample:
                    # Always generate a fresh P
                    P = self._sample_orthogonal_P(m, rank_r, param.data.device, param.data.dtype)
                    self.P_matrices[name] = P
                else:
                    P = self.P_matrices[name]

                # Standard Gaussian sampling
                u = torch.randn(rank_r, n, device=param.data.device, dtype=param.data.dtype)
    ...
    def lowdim_zo_update(self):
        ...
        for name, param in self.named_parameters_to_optim:
            if param.data.ndim >= 2:
                ...
                lowdim_rge = torch.zeros(rank_r, param.data.shape[1], device=param.data.device, dtype=param.data.dtype)
                ...
                if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
                    G.add_(args.weight_decay, param.data)
