...
class MeZOTrainer(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        self.gradient_spectrum_recorder = GradientSpectrumRecorder(self.args, self.model, method_name="mezo")

    def _get_learning_rate(self):
        return trainer_get_learning_rate(self)

    def _maybe_record_zo_spectrum(self):
        step = self.state.global_step + 1
        if not self.gradient_spectrum_recorder.should_capture(step):
            return

        torch.manual_seed(self.zo_random_seed)
        sparsity = getattr(self.args, "sparsity", 1.0)
        named_tensors = {}
        for name, param in self.named_parameters_to_optim:
            z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
            if sparsity < 1.0:
                mask_probs = torch.full_like(z, sparsity)
                mask = torch.bernoulli(mask_probs)
                z = z * mask
            if self.gradient_spectrum_recorder.is_selected(name):
                named_tensors[name] = self.projected_grad * z

        self.gradient_spectrum_recorder.capture_from_named_tensors(
            step=step,
            source="mezo_estimated_gradient",
            named_tensors=named_tensors,
            extra={
                "projected_grad": float(self.projected_grad),
                "zo_eps": float(self.args.zo_eps),
            },
        )
    ...
    def zo_perturb_parameters(self, random_seed=None, scaling_factor=1):
        ...
        # Default sparsity to 1.0 if not present in args
        sparsity = getattr(self.args, "sparsity", 1.0)
        ...
        for name, param in self.named_parameters_to_optim:
            ...
            # 2. Apply Sparsity Mask if needed
            if sparsity < 1.0:
                # Create a random mask where each element is 1 with probability 'sparsity'
                # torch.bernoulli requires float input for probabilities
                mask_probs = torch.full_like(z, sparsity)
                mask = torch.bernoulli(mask_probs)
                z = z * mask
    ...
    def zo_forward(self, model, inputs):
        ...
        with torch.inference_mode():
            ...
            # Assuming compute_loss_context_manager and compute_loss exist in parent/mixin
    ...
    def zo_step(self, model, inputs):
        ...
        # First function evaluation: theta + z * eps
        self.zo_perturb_parameters(scaling_factor=1)
        loss1 = self.zo_forward(model, inputs)

        # Second function evaluation: theta - z * eps
        # We move from (theta + z*eps) to (theta - z*eps), so we subtract 2*z*eps
        self.zo_perturb_parameters(scaling_factor=-2)
        loss2 = self.zo_forward(model, inputs)

        self.projected_grad = ((loss1 - loss2) / (2 * self.args.zo_eps)).item()
        self._maybe_record_zo_spectrum()

        # No gradient accumulation support
        assert self.args.gradient_accumulation_steps == 1

        # Reset model back to its parameters at start of step
        # We move from (theta - z*eps) back to theta, so we add 1*z*eps
        self.zo_perturb_parameters(scaling_factor=1)
    ...
    def zo_update(self, model):
        ...
        # Reset the random seed for sampling zs (and masks)
        # This ensures 'z' and 'mask' generated here are identical to zo_perturb_parameters
        torch.manual_seed(self.zo_random_seed)     
        ...
        for name, param in self.named_parameters_to_optim:
            ...
            # 2. Resample Mask (Deterministic due to manual_seed)
            if sparsity < 1.0:
                mask_probs = torch.full_like(z, sparsity)
                mask = torch.bernoulli(mask_probs)
                z = z * mask
